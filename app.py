...
# Flaskオブジェクトの生成
app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret key here'  # ランダムな値を作成する
auth = HTTPDigestAuth()
...
# 認証に成功したら以下の処理を実行する
def index():
    return render_template('sabi.html')


@ app.route('/sabi', methods=['GET', 'POST'])
def upload_file():
    ...
    if request.method == 'POST':
        ...
        return render_template('sabi.html', filepath=filepath, predict=predict)
